...
save.image("5장_1111B.RData") # save(list=ls(), file="5장.RData")
rm(list=ls())
load("5장.RData")  
...
plot(x=fitted(fit), y=rstandard(fit), ylim=c(-2,3), lwd=4, col=4, 
     main="standardized residual")
abline(h=0, col=2, lwd=2)

plot(x=fitted(fit), y=rstudent(fit), ylim=c(-2,3), lwd=4, col=4, 
     main="studentized residual")
abline(h=0, col=2, lwd=2)
...
?qqPlot
qqPlot(x=fit$residuals, distribution = "norm", 
       main="QQ Plot", points.col=4, line.lwd =4)    #qq plot for studentized resid
qqline(fit$residuals)

x11()
?qqnorm
qqnorm(y=fit$residuals, main="QQ Plot", col=4, lwd=2)
qqline(y=fit$residuals, col=2, lwd=2)

help(model.matrix)
help(hat)
h=stats::hat(model.matrix(fit))
h
...
plot(fit, which=2,  col=4, lwd=2)

install.packages('car')
library(car)
?car::qqPlot()
car::qqPlot(x=studres(fit), main="QQ Plot")  # t 분포 Q-Q plot for studentized resid
car::qqPlot(fit, main="QQ Plot")  # t 분포 Q-Q plot for studentized resid
